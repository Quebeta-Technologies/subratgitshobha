...
const navLinks = [
  ...
  { label: "About", href: "/about" },
  {
    ...
    dropdown: [
      { icon: Wind, label: "MDI Inhalers", href: "/pharmaceuticals?tab=mdi-inhalers", color: "#62C7F5" },
      { icon: Shield, label: "B-Lactum Antibiotics", href: "/pharmaceuticals?tab=b-lactum-antibiotics", color: "#9DCD4A" },
      { icon: Syringe, label: "Injectables", href: "/pharmaceuticals?tab=injectables", color: "#E84D6C" },
      { icon: Microscope, label: "Oncology — Anti Cancer", href: "/pharmaceuticals?tab=oncology", color: "#0738A6" },
      { icon: Pill, label: "Cephalosporins", href: "/pharmaceuticals?tab=cephalosporins", color: "#7A1F7A" },
    ],
  },
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-screen w-[300px] bg-white z-[9999] flex flex-col shadow-2xl"
          >
            {/* Drawer Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#E9EEF5] flex-shrink-0">
              <Logo className="h-9 w-auto" />
              <button
                onClick={onClose}
                className="p-2 text-[#12233D] hover:bg-[#F7FAFD] rounded-lg transition-colors"
                  ...
                  {l.dropdown ? (
                    <div>
                      <button
                        ...
                        className="w-full py-3.5 flex items-center justify-between text-[#12233D] font-medium border-b border-[#E9EEF5]"
                      >
                        {l.label}
                        <ChevronDown
                            ...
                            <div className="pl-4 py-2 space-y-1">
                              {l.dropdown.map((sub) => (
                                <a
                                  ...
                                  className="flex items-center gap-3 py-2.5 text-[#4B5563] text-sm hover:text-[#0738A6] transition-colors"
                                >
                    ...
                    <a
                      href={l.href}
                      onClick={onClose}
                      className="block py-3.5 text-[#12233D] font-medium border-b border-[#E9EEF5] hover:text-[#0738A6] transition-colors"
                    >
                      {l.label}
                    </a>
                  )}
            ...
            {/* Footer CTA */}
            <div className="px-5 py-5 border-t border-[#E9EEF5] flex-shrink-0">
              <a
                href="/contact"
                onClick={onClose}
...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Portal drawer — renders outside header so no clipping */}
  ...
  return (
    ...
    >
      ...
      >
        {item.label}
        {hasDropdown && (
          <ChevronDown className={`w-3.5 h-3.5 transition-transform ${hover ? "rotate-180" : ""}`} />
        )}
      </a>
      {hasDropdown && hover && (
